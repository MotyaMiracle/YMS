...
namespace Application.Services.Gates
{
    public class GateService : IGatesService
    {
        ...
        public async Task<bool> CanDriveToGateAsync(string carNumber, CancellationToken token)
        {
            ...
            trip = await _database.Trips.FirstOrDefaultAsync(x => x.Truck.Number == carNumber && x.ArrivalTimePlan.AddMinutes(-30) <= arrivalTime && arrivalTime <= x.ArrivalTimePlan.AddMinutes(30), token);
            if (trip != null)
            {
